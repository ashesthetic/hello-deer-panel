...
class SftFileProcessorService
{
    ...
    /**
     * Process all SFT files for a given date from the pos/data directory
     */
    public function processSftFilesFromPosData(string $date): array
    {
        $formattedDate = \Carbon\Carbon::parse($date)->format('Y-m-d');
        $files = $this->scanPosDataDirectory($formattedDate);
        ...
        $aggregatedData = [
            ...
            'pos_interac_debit' => 0, 'pos_debit_transaction_count' => 0,
            'afd_visa' => 0, 'afd_mastercard' => 0, 'afd_amex' => 0,
            'afd_commercial' => 0, 'afd_up_credit' => 0, 'afd_discover' => 0,
            ...
            'sup_plus_volume' => 0, 'sup_plus_total' => 0,
            'files_processed' => 0, 'files_with_errors' => 0,
            'processed_files' => [], 'errors' => []
        ];

        foreach ($files as $fileInfo) {
            try {
                $fileData = $this->parseSftFileFromPath($fileInfo['path'], $fileInfo['name']);
    ...
    /**
     * Parse an SFT file directly from a filesystem path (no DB record needed)
     */
    private function parseSftFileFromPath(string $filePath, string $fileName): array
    {
        try {
            if (!file_exists($filePath)) {
                return [
